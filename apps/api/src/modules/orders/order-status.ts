...
export type OrderItemStatus = 'pending' | 'ready' | 'shipped' | 'delivered' | 'cancelled' | 'issue'

export type OrderStatus = 'pending' | 'confirmed' | 'processing' | 'shipped' | 'delivered' | 'cancelled' | 'refunded' | 'partially_shipped'
...
export function assertOrderItemTransition(from: OrderItemStatus, to: OrderItemStatus): void {
  ...
  if (!allowed?.includes(to)) {
    throw new BadRequestException(`Transition de statut article invalide : "${from}" → "${to}"`)
  }
}
...
export function assertOrderTransition(from: OrderStatus, to: OrderStatus): void {
  ...
  if (!allowed?.includes(to)) {
    throw new BadRequestException(`Transition de statut commande invalide : "${from}" → "${to}"`)
  }
}
