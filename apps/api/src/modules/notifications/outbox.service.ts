...
export class OutboxService {
  ...
  async createEvent(params: {
    ...
    try {
      ...
      this.logger.warn(
        `createEvent échoué pour ${params.idempotencyKey}: ${(err as Error)?.message}`,
      );
    }
  }
  ...
  async createEventInTx(
    ...
    try {
      ...
      setLogContext('aggregateType', params.aggregateType);
      this.logger.warn(
        `createEventInTx échoué pour ${params.idempotencyKey}: ${(err as Error)?.message}`,
      );
    }
  }
  ...
  async processNextBatch(
    handler: (event: OutboxRow) => Promise<void>,
  ): Promise<number> {
    ...
    for (const event of events) {
      try {
        ...
        this.logger.error(
          `Échec traitement outbox ${event.id} (${event.type}): ${msg}`,
        );
