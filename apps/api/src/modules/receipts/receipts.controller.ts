import { Controller, Get, Post, Put, Param, Query, Body, UseGuards, Req, UnauthorizedException, NotFoundException } from '@nestjs/common'
...
export class ReceiptsController {
  ...
  async download(@Param('id') id: string) {
    ...
    const url = await this.storage.getUrl(receipt.downloadUrl.replace('/uploads/', '').replace(/\\/g, '/'))
    return { url }
  }
...
export class MobileReceiptsController {
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Télécharger le PDF d\'un reçu (client propriétaire)' })
  async download(@Param('id') id: string, @CurrentUser() user: any) {
    const receipt = await this.service.getById(id)
    if (!receipt.downloadUrl) throw new NotFoundException('PDF non disponible')
    if (user?.id) {
      const order = await this.service.getOrderCustomerId(receipt.orderId)
      if (order && order !== user.id) throw new UnauthorizedException('Ce reçu ne vous appartient pas')
    }
    const url = await this.storage.getUrl(receipt.downloadUrl.replace('/uploads/', '').replace(/\\/g, '/'))
    return { url }
  }
