...
describe('ReceiptsService', () => {
  ...
  describe('create', () => {
    it('throws NotFoundException for unknown order', async () => {
      ...
      await expect(
        service.create({ orderId: 'nonexistent', storeId: 's1' }),
    ...
    it('generates sequential unique receipt number via FOR UPDATE counter — scenario 8', async () => {
      ...
      mockTx.select = jest.fn(() => {
        ...
        if (selectCall === 4)
          return makeChain([
            { id: 'pay-1', method: 'orange_money', status: 'captured' },
          ]); // payment
        return makeChain([]);
      });
          ...
          Promise.resolve([
            {
              ...receipt,
              status: 'sent',
              downloadUrl: 'https://storage.example.com/receipt.pdf',
            },
          ]),
        ),
      }));
