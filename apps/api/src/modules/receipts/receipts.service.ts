...
export class ReceiptsService {
  ...
  private async generatePdfBuffer(r: {
    ...
    if (r.storeId) {
      const [s] = await this.db
        ...
        .where(eq(receiptSettings.storeId, r.storeId))
        .limit(1);
      if (s) {
        brand = {
          name: s.brandName || 'ExpressAfri',
          color:
            s.accentColor && /^#[0-9A-Fa-f]{6}$/.test(s.accentColor)
              ? s.accentColor
              : '#f97316',
    ...
    const date = (r.sentAt ?? r.createdAt ?? new Date()).toLocaleDateString(
      'fr-FR',
      {
        day: '2-digit',
        month: 'long',
    ...
    const amount =
      new Intl.NumberFormat('fr-FR').format(Number(r.amount)) +
      ' ' +
      r.currency;
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      if (r.customerPhone)
        doc.text(
          `Téléphone : ${r.customerPhone}`,
          40,
          r.customerEmail ? 150 : 134,
        );
      ...
      if (snapshot?.items && snapshot.items.length > 0) {
        ...
        for (const item of snapshot.items) {
          ...
          doc.text(String(item.quantity), 180, detailY, {
            width: 30,
            align: 'center',
          });
          const pu = new Intl.NumberFormat('fr-FR').format(
        ...
        const fmt = (v: string) =>
          new Intl.NumberFormat('fr-FR').format(Number(v));
        doc.fontSize(9).fillColor('#6b7280');
        const drawRow = (label: string, value: string, y: number) => {
          doc.font('Helvetica').text(label, 40, y, { width: 160 });
          doc
            .font('Helvetica-Bold')
            .text(value, 270, y, { width: 60, align: 'right' });
        };
        drawRow(
          'Sous-total',
          fmt(snapshot.subtotal) + ' ' + r.currency,
          detailY,
        );
        detailY += 16;
        if (Number(snapshot.shippingCost) > 0) {
          drawRow(
            'Livraison',
            fmt(snapshot.shippingCost) + ' ' + r.currency,
            detailY,
          );
          detailY += 16;
        }
        if (Number(snapshot.discountAmount) > 0) {
          drawRow(
            'Remise',
            '-' + fmt(snapshot.discountAmount) + ' ' + r.currency,
            detailY,
          );
          detailY += 16;
        }
        ...
        doc
          .font('Helvetica')
          .text('Moyen de paiement', 40, detailY, { width: 160 });
        doc.font('Helvetica-Bold').text(snapshot.paymentMethod, 270, detailY, {
          width: 60,
          align: 'right',
        });
        detailY += 16;
        doc
          .font('Helvetica')
          .text('Statut du paiement', 40, detailY, { width: 160 });
        doc
          .font('Helvetica-Bold')
          .text(
      ...
      doc
        ...
        .fontSize(8)
        .text(
          'Reçu émis le ' + date + ' — document non modifiable.',
          40,
          footerY,
      ...
      doc
        ...
        .font('Helvetica')
        .fontSize(9)
        .text(brand.footer, 40, footerY, {
          align: 'center',
          width: doc.page.width - 80,
        })
        .text('Ce reçu a été généré automatiquement.', 40, footerY + 14, {
          align: 'center',
          width: doc.page.width - 80,
        });
  ...
  async list(params: {
    ...
    if (params.search)
      conditions.push(
        or(
          like(receipts.orderNumber, `%${params.search}%`),
          like(receipts.customerName, `%${params.search}%`),
        ),
      );
  ...
  async create(data: { orderId: string; storeId: string }) {
    ...
    const where = isUuid
      ...
      : eq(orders.orderNumber, data.orderId);
    ...
    return this.db.transaction(async (tx) => {
      ...
      const nextNumber = lockedSettings?.nextNumber ?? 1;
      ...
      const billAddr = order.billingAddress as {
        name?: string;
        email?: string;
        phone?: string;
      } | null;
  ...
  async send(id: string) {
    ...
    await this.chat.postOrderSystemMessage(
      ...
      receiptMessage(receipt, language),
      {
        ...
        attachmentName: `Reçu-${receipt.orderNumber}.pdf`,
        type: 'pdf',
      },
    );
  ...
  async sendBulk(ids: string[]) {
    ...
    const rows = await this.db
      .select()
      .from(receipts)
      .where(sql`${receipts.id} = ANY(${ids}::uuid[])`);
    for (const r of rows) {
      ...
      if (!downloadUrl) {
        ...
        downloadUrl = await this.storage.save(
          key,
          pdfBuffer,
          'application/pdf',
        );
      ...
      await this.chat.postOrderSystemMessage(
        ...
        {
          attachmentUrl: downloadUrl,
          attachmentName: `Reçu-${r.orderNumber}.pdf`,
          type: 'pdf',
        },
      );
    }
