...
export class ReportsService {
  ...
  async list(params: {
    ...
    if (params.search)
      conditions.push(
        or(
          like(reports.reason, `%${params.search}%`),
          like(reports.description, `%${params.search}%`),
          like(reports.reporterName, `%${params.search}%`),
        ),
      );
  ...
  async assign(id: string, body: { adminId: string }) {
    const [report] = await this.db
      .update(reports)
      .set({ assignedTo: body.adminId, updatedAt: new Date() })
      .where(eq(reports.id, id))
      .returning();
    if (!report) throw new NotFoundException('Signalement introuvable');
    return report;
