...
import { eq, sql, and, like, or, inArray, notInArray, asc, desc } from 'drizzle-orm'
...
export class DeliveryService {
  ...
  async listAssignments(params: { page?: number; limit?: number; deliveryPersonId?: string }) {
    const page = params.page ?? 1
    const limit = params.limit ?? 10
    const offset = (page - 1) * limit
  ...
  async assignDelivery(data: { deliveryPersonId: string; orderId: string }) {
    ...
    if (existing) throw new BadRequestException('Cette commande a déjà une livraison en cours')
