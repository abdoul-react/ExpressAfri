...
import { banners, logos, socialLinks, seoMetadata, paymentMethods, feedSections, staticPages, feedPosts, feedPostLikes } from '../../database/schema/content-cms'
...
const COLOR_HEX: Record<string, string> = {
  ...
  gris: '#8E8E93', grey: '#8E8E93', gray: '#8E8E93',
  rouge: '#E53935', red: '#E53935',
  bordeaux: '#7B1F2B', burgundy: '#7B1F2B',
  rose: '#F06292', pink: '#F06292',
  orange: '#FB8C00',
  jaune: '#FDD835', yellow: '#FDD835',
  or: '#D4AF37', dore: '#D4AF37', gold: '#D4AF37',
  vert: '#43A047', green: '#43A047',
  kaki: '#7C7B46', khaki: '#7C7B46',
  turquoise: '#26C6DA', cyan: '#26C6DA',
  bleu: '#1E88E5', blue: '#1E88E5',
  marine: '#173A5E', navy: '#173A5E',
  violet: '#8E24AA', purple: '#8E24AA', mauve: '#B39DDB',
  marron: '#6D4C41', brun: '#6D4C41', brown: '#6D4C41',
  beige: '#D7C4A3',
  creme: '#F5F0E1', cream: '#F5F0E1',
  argent: '#C0C0C0', silver: '#C0C0C0',
  multicolore: '#FFFFFF', multicolor: '#FFFFFF',
}
...
export class MobileService {
  ...
  async register(data: { firstName: string; lastName: string; email: string; phone?: string; password: string }) {
    const existing = await this.db.select().from(customers).where(eq(customers.email, data.email)).limit(1)
    if (existing.length) throw new ConflictException('Cet email est déjà utilisé')
  ...
  async login(email: string, password: string) {
    const [customer] = await this.db.select().from(customers).where(eq(customers.email, email)).limit(1)
    if (!customer) throw new UnauthorizedException('Email ou mot de passe incorrect')
    if (!customer.passwordHash) throw new UnauthorizedException('Compte sans mot de passe (guest/OTP)')
  ...
  async verifyOtp(contact: string, code: string) {
    ...
    if (!customer) {
      ...
      const [c] = await this.db.insert(customers).values({
        storeId: SYSTEM_STORE_ID,
  ...
  async refreshToken(token: string) {
    try {
      ...
      const [customer] = await this.db.select().from(customers).where(eq(customers.id, payload.sub)).limit(1)
  ...
  async socialLogin(provider: string, data: { email?: string; name?: string; id?: string }) {
    let customer = data.email
      ? await this.db.select().from(customers).where(eq(customers.email, data.email)).limit(1).then(r => r[0])
      : null
  ...
  async getProfile(customerId: string) {
    const [c] = await this.db.select().from(customers).where(eq(customers.id, customerId)).limit(1)
    if (!c) throw new NotFoundException('Client introuvable')
    return this.toProfile(c)
  }

  async updateProfile(customerId: string, data: { name?: string; firstName?: string; lastName?: string; phone?: string; avatar?: string; gender?: string; birthYear?: number; language?: string }) {
    ...
    if (data.birthYear !== undefined) patch.birthYear = Number(data.birthYear) || null
    if (data.language !== undefined) patch.language = data.language

    const [c] = await this.db.update(customers).set(patch)
  ...
  async getProducts(query: { categoryId?: string; search?: string; limit?: number; offset?: number }) {
    ...
    const allImages = productIds.length
      ? await this.db.select().from(productImages).where(inArray(productImages.productId, productIds)).orderBy(productImages.sortOrder)
      : []
    const allVariants = productIds.length
      ? await this.db.select().from(productVariants).where(inArray(productVariants.productId, productIds))
      : []
    ...
    return rows.map(p => this.toMobileProduct(p, imagesByProduct.get(p.id) ?? [], variantsByProduct.get(p.id) ?? []))
  }
  ...
  async getProductById(id: string) {
    ...
    const imgs = await this.db.select().from(productImages).where(eq(productImages.productId, id)).orderBy(productImages.sortOrder)
    ...
    }).from(productReviews).where(and(eq(productReviews.productId, id), eq(productReviews.isActive, true)))

    const product = this.toMobileProduct(p, imgs.map(i => i.url), variants)
  ...
  async getProductReviews(productId: string) {
    ...
    return rows.map(r => ({
      ...
      authorName: `${r.firstName ?? ''} ${(r.lastName ?? '').charAt(0)}${r.lastName ? '.' : ''}`.trim() || 'Client',
      authorAvatar: r.avatar ?? '',
    }))
  }

  async createProductReview(customerId: string, productId: string, data: { rating: number; title?: string; content?: string }) {
    ...
    const [product] = await this.db.select({ id: products.id, storeId: products.storeId }).from(products)
      .where(eq(products.id, productId)).limit(1)
    if (!product) throw new NotFoundException('Produit introuvable')

    // Un seul avis par client et par produit : mise à jour si déjà existant
    const [existing] = await this.db.select({ id: productReviews.id }).from(productReviews)
      .where(and(eq(productReviews.productId, productId), eq(productReviews.customerId, customerId))).limit(1)
    if (existing) {
      const [updated] = await this.db.update(productReviews)
        .set({ rating, title: data.title ?? null, content: data.content ?? null, updatedAt: new Date() })
        .where(eq(productReviews.id, existing.id)).returning()
      return { id: updated.id, updated: true }
    }

    // Avis vérifié si le client a réellement commandé ce produit
    const [purchase] = await this.db.select({ id: orderItems.id }).from(orderItems)
      .innerJoin(orders, eq(orderItems.orderId, orders.id))
      .where(and(eq(orderItems.productId, productId), eq(orders.customerId, customerId))).limit(1)

    const [created] = await this.db.insert(productReviews).values({
  ...
  async getCategories() {
    const rows = await this.db.select().from(categories).where(eq(categories.isActive, true)).orderBy(categories.name)
    return rows.map(c => ({
      id: c.id,
      name: c.name,
  ...
  async getCategoryChildren(parentId: string) {
    const rows = await this.db
      .select()
      .from(categories)
      .where(and(eq(categories.isActive, true), eq(categories.parentId as any, parentId)))
  ...
  private toMobileProduct(p: any, images: string[], variants: any[] = []) {
    ...
    const discount = origPrice && origPrice > price ? Math.round((1 - price / origPrice) * 100) : undefined
  ...
  private toVariantGroups(variants: any[]) {
    ...
    const hasAttributed = variants.some((v) => Array.isArray(v.attributes) && v.attributes.length)
    const isSingleDefault = variants.length <= 1 && !hasAttributed

    const groups = new Map<string, { key: string; labelKey: string; options: Map<string, { id: string; label: string; swatch?: string; image?: string; stock: number }> }>()
    for (const v of variants) {
      if (isSingleDefault) continue
      const attrs: any[] = Array.isArray(v.attributes) && v.attributes.length
        ? v.attributes
        : (v.label ? [{ name: 'Variante', value: v.label }] : [])
      for (const a of attrs) {
        if (!a?.name || !a?.value) continue
        const key = String(a.name)
        let group = groups.get(key)
        if (!group) {
          group = { key, labelKey: I18N_KEYS[key.toLowerCase()] ?? key, options: new Map() }
          groups.set(key, group)
        }
        const value = String(a.value)
        if (!group.options.has(value)) {
          const swatch = COLOR_ATTR.has(key.toLowerCase()) ? colorToHex(value) : undefined
          group.options.set(value, { id: value, label: value, swatch, image: v.imageUrl ?? undefined, stock: v.stock ?? 0 })
        } else {
    ...
    return [...groups.values()].map(g => ({ key: g.key, labelKey: g.labelKey, options: [...g.options.values()] }))
  }
  ...
  async getBanners(screen?: string) {
    ...
    if (screen) {
      ...
      conditions.push(eq(banners.screen, screen as typeof validScreens[number]))
    }
    const rows = await this.db.select().from(banners).where(
      and(...conditions),
  ...
  async getFeedPosts(customerId?: string) {
    ...
    return rows.map(({ post, likes, likedByMe }) => ({
      ...
      image: post.mediaType === 'video' ? (post.thumbnailUrl ?? post.mediaUrl) : post.mediaUrl,
  ...
  async toggleFeedPostLike(postId: string, customerId: string) {
    const [post] = await this.db.select({ id: feedPosts.id }).from(feedPosts)
      .where(eq(feedPosts.id, postId)).limit(1)
    if (!post) throw new NotFoundException('Publication introuvable')

    const [existing] = await this.db.select({ id: feedPostLikes.id }).from(feedPostLikes)
      .where(and(eq(feedPostLikes.postId, postId), eq(feedPostLikes.customerId, customerId))).limit(1)

    if (existing) {
      await this.db.delete(feedPostLikes).where(eq(feedPostLikes.id, existing.id))
    } else {
      await this.db.insert(feedPostLikes).values({ postId, customerId }).onConflictDoNothing()
    }

    const [{ count }] = await this.db.select({ count: sql<number>`count(*)` }).from(feedPostLikes)
  ...
  async getFeedSectionProducts(sectionId: string) {
    const [section] = await this.db.select().from(feedSections)
      .where(and(eq(feedSections.id, sectionId), eq(feedSections.isActive, true))).limit(1)
    if (!section) throw new NotFoundException('Section introuvable')

    const tagValue = `section:${section.id}`
    let productList = await this.db.select().from(products)
      .where(and(
    ...
    if (productList.length === 0) {
      productList = await this.db.select().from(products)
        .where(eq(products.status, 'active'))
        .orderBy(desc(products.createdAt)).limit(30)
    }
    ...
    return {
      ...
      items: productList.map(p => this.toMobileProduct(p, imagesByProduct.get(p.id) ?? [])),
    }
  }
  ...
  async getFeedSections() {
    ...
    const enriched = await Promise.all(rows.map(async (section) => {
      ...
      const productList = sectionProducts.length > 0
        ? sectionProducts
        : await this.db.select().from(products).where(eq(products.status, 'active')).orderBy(desc(products.createdAt)).limit(6)
  ...
  async getShippingCountries(): Promise<string[]> {
    const zones = await this.db.select({ countries: shippingZones.countries })
      .from(shippingZones)
    ...
    for (const z of zones) {
      ...
      for (const code of list) {
        if (typeof code === 'string' && code.length === 2) set.add(code.toUpperCase())
      }
    }
    return [...set]
  }
  ...
  async getShippingQuote(input: { country?: string; subtotal?: number }) {
    ...
    if (country) {
      const zones = await this.db.select().from(shippingZones)
        .where(eq(shippingZones.isActive, true))
        .orderBy(desc(shippingZones.priority))
      const zone = zones.find((z) => {
        const list = Array.isArray(z.countries) ? z.countries : []
        return list.some((c) => typeof c === 'string' && c.toUpperCase() === country)
      })

      if (zone) {
        const methods = await this.db.select().from(shippingMethods)
          .where(and(eq(shippingMethods.zoneId, zone.id), eq(shippingMethods.isActive, true)))
    ...
    const rows = await this.db.select().from(appSettings)
      .where(inArray(appSettings.key, ['commerce.baseShippingFee', 'commerce.freeShippingThreshold']))
  ...
  async getCartRecommendations() {
    const sections = await this.db
      .select()
      .from(feedSections)
      .where(and(eq(feedSections.isActive, true), eq(feedSections.type, 'products')))
    const target = sections.find(s => (s.data as any)?.cartRecommendations === true)
    ...
    return productList.map(p => this.toMobileProduct(p, imagesByProduct.get(p.id) ?? []))
  }
  ...
  async getHomeShortcuts() {
    const rows = await this.db
      .select()
      .from(contentBlocks)
      .where(and(eq(contentBlocks.groupName, 'shortcuts'), eq(contentBlocks.isActive, true)))
      .orderBy(contentBlocks.key)
  ...
  async listStaticPages() {
    return this.db
      .select({ slug: staticPages.slug, title: staticPages.title, updatedAt: staticPages.updatedAt })
      .from(staticPages)
  ...
  async searchByImage(_imagePath: string) {
    ...
    return rows.map((p) => this.toMobileProduct(p, imagesByProduct.get(p.id) ?? []))
  }
  ...
  async followStore(customerId: string, storeId: string) {
    const [store] = await this.db.select({ id: stores.id }).from(stores).where(eq(stores.id, storeId)).limit(1)
    if (!store) throw new NotFoundException('Boutique introuvable')
    // Idempotent : le doublon est ignoré grâce à la contrainte unique
    await this.db.insert(storeFollows).values({ customerId, storeId }).onConflictDoNothing()
    return { following: true }
  }

  async unfollowStore(customerId: string, storeId: string) {
    await this.db.delete(storeFollows)
      .where(and(eq(storeFollows.customerId, customerId), eq(storeFollows.storeId, storeId)))
  ...
  async getCustomerConversations(customerId: string) {
    ...
    return Promise.all(convs.map(async (conv) => {
      ...
      return {
        ...
        lastMessage: lastMsg
          ? (lastMsg.deletedAt ? 'Message supprimé'
            : lastMsg.type === 'image' ? (lastMsg.content ? `📷 ${lastMsg.content}` : '📷 Photo')
            : lastMsg.type === 'video' ? (lastMsg.content ? `🎥 ${lastMsg.content}` : '🎥 Vidéo')
            : lastMsg.type === 'audio' ? '🎤 Message vocal'
            : lastMsg.type === 'pdf' ? `📄 ${lastMsg.attachmentName || 'Document'}`
            : lastMsg.content ?? '')
          : '',
  ...
  async getWallet(customerId: string) {
    const [points] = await this.db.select().from(loyaltyPoints)
      .where(eq(loyaltyPoints.customerId, customerId)).limit(1)

    const [savings] = await this.db
      .select({ total: sql<string>`coalesce(sum(${orders.discountAmount}), 0)` })
      .from(orders)
