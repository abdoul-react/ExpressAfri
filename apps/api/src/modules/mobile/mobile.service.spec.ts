...
describe('MobileService — OTP (scenario 12)', () => {
  ...
  describe('requestOtp', () => {
    it('rejects request if more than 3 OTPs requested in last hour', async () => {
      ...
      await expect(service.requestOtp('test@example.com', 'email'))
        .rejects.toThrow(BadRequestException)
    })

    it('creates OTP code when under rate limit', async () => {
      mockDb.select = jest.fn(() => makeChain([{ count: 0 }]))
      const valuesChain = { onConflictDoUpdate: jest.fn().mockResolvedValue(undefined) }
  ...
  describe('verifyOtp', () => {
    ...
    it('rejects already used code (scenario 12 — reused)', async () => {
      const stored = { codeHash: validCodeHash, usedAt: new Date(), expiresAt: futureDate, attempts: 0, maxAttempts: 5 }
    ...
    it('rejects expired code (scenario 12 — expired)', async () => {
      const stored = { codeHash: validCodeHash, usedAt: null, expiresAt: pastDate, attempts: 0, maxAttempts: 5 }
      mockDb.select = jest.fn(() => makeChain([stored]))
    ...
    it('rejects after max attempts exceeded (scenario 12 — 5 essais)', async () => {
      const stored = { codeHash: validCodeHash, usedAt: null, expiresAt: futureDate, attempts: 5, maxAttempts: 5 }
      mockDb.select = jest.fn(() => makeChain([stored]))
    ...
    it('rejects incorrect code and increments attempts', async () => {
      const stored = { codeHash: validCodeHash, usedAt: null, expiresAt: futureDate, attempts: 0, maxAttempts: 5 }
      mockDb.select = jest.fn(() => makeChain([stored]))
      ...
      await expect(service.verifyOtp('test@example.com', 'wrong-code'))
        .rejects.toThrow(UnauthorizedException)
    ...
    it('accepts valid code and marks as used', async () => {
      const stored = { codeHash: validCodeHash, usedAt: null, expiresAt: futureDate, attempts: 0, maxAttempts: 5 }
      const customer = { id: 'cust-1', storeId: 's1', firstName: 'Test', lastName: 'User', email: 'test@example.com' }
