...
export class AdminMessagesService {
  ...
  async listChatConversations(params: {
    ...
    if (params.storeId)
      conditions.push(eq(conversations.storeId, params.storeId));
    ...
    const data = rows.map((r) => {
      ...
      return {
        ...
        lastMessageAt:
          last?.createdAt?.toISOString() ?? r.updatedAt?.toISOString() ?? '',
  ...
  async getChatAwaitingCount(storeId?: string): Promise<{ count: number }> {
    const storeCondition = storeId
      ? sql`AND c.store_id = ${storeId}`
      : sql``;
    const result = await this.db.execute(sql`
      ...
      WHERE c.status = 'open'
        AND last_msg.sender_role = 'customer'
        ${storeCondition}
    `);
    return { count: Number((result.rows[0] as any)?.count ?? 0) };
  }
  ...
  async getChatConversation(id: string, restrictToStoreId?: string) {
    ...
    if (restrictToStoreId && conv.storeId !== restrictToStoreId) {
      throw new ForbiddenException(
        'Cette conversation appartient à une autre boutique',
      );
    }
    ...
    const [msgList, [customer], [store]] = await Promise.all([
      ...
      this.db
        .select({
          ...
          chatBlockedAt: customers.chatBlockedAt,
        })
        .from(customers)
  ...
  async replyChatConversation(
    ...
    if (type !== 'text' && !payload.attachmentUrl)
      throw new BadRequestException('Pièce jointe manquante');
    ...
    if (payload.replyToId) {
      const [target] = await this.db
        ...
        .where(
          and(
            eq(messages.id, payload.replyToId),
  ...
  async updateChatStatus(
    ...
    if (restrictToStoreId) {
      const [existing] = await this.db
        .select({ storeId: conversations.storeId })
        .from(conversations)
        .where(eq(conversations.id, id))
        .limit(1);
  ...
  async getChatConversationMedia(
    conversationId: string,
    restrictToStoreId?: string,
  ) {
    const [conv] = await this.db
      .select({ id: conversations.id, storeId: conversations.storeId })
      .from(conversations)
      .where(eq(conversations.id, conversationId))
      .limit(1);
    if (!conv) throw new NotFoundException('Conversation introuvable');
  ...
  async list(params: {
    ...
    const conditions = [];
    if (params.status)
      conditions.push(eq(adminTickets.status, params.status as any));
    if (params.priority)
      conditions.push(eq(adminTickets.priority, params.priority as any));
    if (params.search)
      conditions.push(
        or(
    ...
    const [data, [{ count }]] = await Promise.all([
      this.db
        ...
        .limit(limit)
        .offset(offset)
        .orderBy(desc(adminTickets.updatedAt)),
      this.db
        .select({ count: sql<number>`count(*)` })
        .from(adminTickets)
        .where(where),
  ...
  async listInternalMessages(params: {
    page?: number;
    limit?: number;
    search?: string;
  }) {
  ...
  async sendInternalMessage(input: {
    toAdminId: string;
    toAdminName: string;
    subject: string;
    content: string;
