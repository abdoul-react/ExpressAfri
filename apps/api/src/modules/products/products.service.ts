...
import { eq, like, or, and, sql, inArray, asc, desc } from 'drizzle-orm';
...
export class ProductsService {
  ...
  async list(params: {
    ...
    if (params.moderationStatus)
      conditions.push(eq(products.moderationStatus, params.moderationStatus));

    const sortColumn =
      ({ price: products.price, name: products.name, createdAt: products.createdAt } as any)[
    ...
    // Batch-fetch store and category names (no N+1)
    const storeIds = [...new Set(data.map((p) => p.storeId).filter(Boolean))] as string[];
    const categoryIds = [...new Set(data.map((p) => p.categoryId).filter(Boolean))] as string[];
    const [allStores, allCats] = await Promise.all([
      storeIds.length
        ? this.db.select({ id: stores.id, name: stores.name }).from(stores).where(inArray(stores.id, storeIds))
        : Promise.resolve([]),
      categoryIds.length
        ? this.db.select({ id: categories.id, name: categories.name }).from(categories).where(inArray(categories.id, categoryIds))
        : Promise.resolve([]),
    ]);
    const storeNameById = new Map(allStores.map((s) => [s.id, s.name]));
    const categoryNameById = new Map(allCats.map((c) => [c.id, c.name]));
    ...
    const dataWithImages = data.map((p) => ({
      ...
      variants: [],
      storeName: storeNameById.get(p.storeId ?? '') ?? null,
  ...
  async getById(id: string) {
    ...
    const defaultVariant =
      variants.find((v) => !(v.attributes as any[])?.length) ?? variants[0];
  ...
  async create(data: CreateProductDto & { storeId?: string; slug?: string }) {
    ...
    if (variants?.length) {
      await this.db
        ...
        .values(
          variants.map((v: ProductVariantDto, idx: number) =>
            toVariantRow(v, product.id, productData.storeId, idx),
          ),
        );
    } else {
    ...
    if (images?.length) {
      await this.db
        ...
        .values(images.map((img: any) => ({ ...img, productId: product.id })));
    }
  ...
  async update(id: string, data: UpdateProductDto & { storeId?: string }) {
    ...
    if (variants?.length) {
      ...
      await this.db
        ...
        .values(
          variants.map((v: ProductVariantDto, idx: number) =>
            toVariantRow(v, id, product.storeId, idx),
          ),
        );
    } else if (
      variants !== undefined ||
      ...
      const existing = await this.db
        ...
        .where(eq(productVariants.productId, id));
      const defaultVariant =
        existing.find((v) => !(v.attributes as any[])?.length) ?? existing[0];
      const row = toVariantRow(
        {
          sku: sku || defaultVariant?.sku,
          label: productData.name ?? product.name,
          attributes: [],
          price: null,
          stock: Number(stock) || 0,
          isActive: true,
        ...
        await this.db
          ...
          .where(
            and(
              ...
              sql`${productVariants.id} <> ${defaultVariant.id}`,
            ),
          );
        await this.db
    ...
    if (images) {
      ...
      if (images.length)
        await this.db
          .insert(productImages)
          .values(images.map((img: any) => ({ ...img, productId: id })));
    }
