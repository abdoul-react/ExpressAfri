...
} from '../../database/schema/affiliates';
...
export class AffiliatesService {
  ...
  async list(params: {
    ...
    if (params.search)
      conditions.push(
        or(
          like(affiliates.name, `%${params.search}%`),
          like(affiliates.email, `%${params.search}%`),
          like(affiliates.country, `%${params.search}%`),
        ),
      );
  ...
  async listCommissions(params: {
    ...
    if (params.affiliateId)
      conditions.push(eq(affiliateCommissions.affiliateId, params.affiliateId));
    if (params.status)
      conditions.push(eq(affiliateCommissions.status, params.status));
  ...
  async getSummary() {
    const [totalAgg] = await this.db
      .select({ count: sql<number>`count(*)` })
      .from(affiliates);
    const [activeAgg] = await this.db
      .select({ count: sql<number>`count(*)` })
      .from(affiliates)
      .where(eq(affiliates.status, 'active'));
    const [pendingAgg] = await this.db
      .select({ total: sql<string>`coalesce(sum(commission_amount), '0')` })
      .from(affiliateCommissions)
    ...
    const [paidAgg] = await this.db
      ...
      .where(
        and(
          ...
          sql`paid_at >= ${firstOfMonth.toISOString()}`,
        ),
      );
  ...
  async listCodes(params: { page?: number; limit?: number; affiliateId?: string }) {
    const page = params.page ?? 1;
    const limit = params.limit ?? 10;
    const offset = (page - 1) * limit;
    const conditions = [];
  ...
  async createCode(data: any) {
    const [code] = await this.db.insert(coupons).values(data).returning();
    return code;
  }

  async updateCode(id: string, data: any) {
    const [code] = await this.db.update(coupons)
      .set({ ...data, updatedAt: new Date() })
  ...
  async deleteCode(id: string) {
    const [code] = await this.db.select().from(coupons).where(eq(coupons.id, id)).limit(1);
    if (!code) throw new NotFoundException('Code introuvable');
