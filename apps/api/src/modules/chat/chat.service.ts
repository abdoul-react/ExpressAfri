...
export class ChatService {
  ...
  async postOrderSystemMessage(
    ...
    try {
      ...
      console.error(
        `[ChatService] postOrderSystemMessage échoué pour orderId=${orderId}:`,
        e,
      );
    }
  }
  ...
  async sendMessage(
    ...
    if (sender.role === 'customer') {
      const [c] = await this.db
        .select({ chatBlockedAt: customers.chatBlockedAt })
        .from(customers)
        .where(eq(customers.id, sender.id))
        .limit(1);
      if (c?.chatBlockedAt)
        throw new ForbiddenException('Vous ne pouvez plus envoyer de messages');
    }
