...
export class ChatController {
  ...
  @ApiOperation({
    summary:
      'Créer une conversation (storeId ou orderId — dédupliquée par commande)',
  })
  async createConversation(
    @CurrentUser() user: any,
    @Body() body: { storeId?: string; orderId?: string; subject?: string },
  ) {
    if (!user?.id) throw new UnauthorizedException('Connexion requise');
  ...
  @ApiOperation({
    summary:
      'Envoyer un message (texte et/ou pièce jointe, réponse à un message)',
  })
  async sendMessage(
    @CurrentUser() user: any,
    @Param('id') id: string,
  ...
  @UseInterceptors(
    FileInterceptor('file', {
      ...
      fileFilter: (_req, file, cb) => {
        ...
        if (
          !file.mimetype.match(
            /^(image\/(png|jpe?g|webp|gif)|video\/(mp4|quicktime|webm|3gpp)|application\/pdf|audio\/(mpeg|mp4|m4a|x-m4a|aac|wav|webm|3gpp))$/,
          )
        ) {
          cb(
            new BadRequestException(
              'Format non accepté (image, vidéo, pdf, audio)',
            ),
            false,
          );
        } else {
  ...
  @ApiOperation({
    summary: 'Uploader une pièce jointe de chat (retourne url, name, type)',
  })
  ...
  @ApiOperation({
    summary: 'Médias, documents et liens échangés dans une conversation',
  })
