...
describe('PaymentWebhookService', () => {
  ...
  beforeEach(async () => {
    ...
    mockTx = { select: jest.fn(() => makeChain([])), update: jest.fn(() => makeChain()) }
    mockDb = {
  ...
  describe('processWebhook', () => {
    it('returns error for unsupported provider', async () => {
      const result = await service.processWebhook('unknown', Buffer.from('{}'), 'sig')
      expect(result.status).toBe('error')
      expect(result.message).toContain('non supporté')
    })

    it('returns error for invalid signature', async () => {
      const result = await service.processWebhook('mock', Buffer.from('{}'), 'wrong-sig')
      expect(result.status).toBe('error')
    ...
    it('returns error for unknown payment', async () => {
      ...
      const raw = Buffer.from(JSON.stringify({ eventId: 'e1', providerPaymentId: 'pp-missing', status: 'captured' }))
      const result = await service.processWebhook('mock', raw, 'any-signature-will-fail-here')
      expect(result.status).toBe('error')
    })

    it('processes valid webhook', async () => {
      const payment = { id: 'pay-1', orderId: 'order-1', providerPaymentId: 'pp-1', amount: '100.00', currency: 'XOF', status: 'pending', webhookEventId: null }
      mockTx.select = jest.fn(() => makeChain([payment]))
      mockTx.update = jest.fn(() => makeChain())
      mockDb.transaction.mockImplementation(async (cb: any) => cb(mockTx))

      const crypto = require('crypto')
      const payload = { eventId: 'e-valid', providerPaymentId: 'pp-1', status: 'captured' }
    ...
    it('ignores duplicate webhook event idempotently', async () => {
      const payment = { id: 'pay-2', orderId: 'order-2', providerPaymentId: 'pp-2', amount: '100.00', currency: 'XOF', status: 'captured', webhookEventId: 'e-dup' }
      mockTx.select = jest.fn(() => makeChain([payment]))
      mockDb.transaction.mockImplementation(async (cb: any) => cb(mockTx))

      const crypto = require('crypto')
      const payload = { eventId: 'e-dup', providerPaymentId: 'pp-2', status: 'captured' }
      const raw = Buffer.from(JSON.stringify(payload), 'utf8')
      const hmac = crypto.createHmac('sha256', MOCK_SECRET)
      hmac.update(raw)
      const sig = hmac.digest('hex')
