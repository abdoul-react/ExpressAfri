import { Controller, Get, Post, Param, Query, Body, UseGuards, Req, Headers } from '@nestjs/common'
...
export class PaymentsController {
  ...
  async list(@Query() query: { page?: number; limit?: number; search?: string; status?: string; method?: string; orderId?: string }) {
  ...
  async create(@Body() body: { orderId: string; storeId: string; amount: string; method?: string; currency?: string; transactionId?: string }) {
    return this.service.create(body)
  }

  @Post(':id/refund')
  @ApiOperation({ summary: 'Rembourser un paiement' })
  async refund(@Param('id') id: string, @Body() body: { amount: number; reason?: string }) { return this.service.refund(id, body) }
