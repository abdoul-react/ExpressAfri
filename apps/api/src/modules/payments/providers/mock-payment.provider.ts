...
export class MockPaymentProvider implements PaymentProvider {
  ...
  async initialize(input: {
    ...
    const checkoutUrl = input.returnUrl ? `${input.returnUrl}?providerPaymentId=${providerPaymentId}` : undefined
    // Default to 'pending' for card flows, 'authorized' for express flows
    const status = input.method === 'card' ? 'pending' : 'authorized'
    this.logger.debug(`Initialized mock payment ${providerPaymentId} status=${status}`)
  ...
  parseWebhook(rawBody: Buffer): {
    ...
    try {
      ...
      const providerPaymentId = parsed.providerPaymentId ?? parsed.provider_payment_id ?? ''
      const status = (parsed.status ?? 'captured') as 'authorized' | 'captured' | 'failed' | 'refunded'
      return { eventId, providerPaymentId, status }
    } catch (err) {
      this.logger.error('Failed to parse webhook payload', err as Error)
      return { eventId: `evt_${Date.now()}`, providerPaymentId: '', status: 'failed' }
    }
  }
}
