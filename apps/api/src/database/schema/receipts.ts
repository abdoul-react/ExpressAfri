...
export const receipts = pgTable(
  ...
  (table) => ({
    receiptsStoreYearSeqUnique: uniqueIndex('receipts_store_year_seq_unique')
      ...
      .where(
        sql`${table.fiscalYear} IS NOT NULL AND ${table.sequenceNumber} IS NOT NULL`,
      ),
    receiptsOrderIdUnique: uniqueIndex('receipts_order_id_unique').on(
      table.orderId,
    ),
  }),
);
...
export const receiptSettings = pgTable(
  ...
  (table) => ({
    receiptSettingsStoreYearUnique: uniqueIndex(
      'receipt_settings_store_year_unique',
