...
const MAGIC_BYTES: { ext: string; mime: string; bytes: number[]; offset: number }[] = [
  ...
  { ext: 'ico', mime: 'image/x-icon', bytes: [0x00, 0x00, 0x01, 0x00], offset: 0 },
  { ext: 'mp4', mime: 'video/mp4', bytes: [0x66, 0x74, 0x79, 0x70], offset: 4 },
  { ext: 'webm', mime: 'video/webm', bytes: [0x1A, 0x45, 0xDF, 0xA3], offset: 0 },
  ...
  { ext: '3gp', mime: 'video/3gpp', bytes: [0x66, 0x74, 0x79, 0x70], offset: 4 },
  { ext: 'mov', mime: 'video/quicktime', bytes: [0x66, 0x74, 0x79, 0x70], offset: 4 },
]
...
export function validateFileContent(filePath: string, allowedMimePrefix: string): void {
  ...
  const candidates = MAGIC_BYTES.filter(
    (s) => s.mime.startsWith(category) || s.mime === allowedMimePrefix || allowedMimePrefix.startsWith(s.mime),
  )
