...
async function main() {
  const admins = await pool.query(
    'SELECT id, name, role, is_super_admin FROM admins ORDER BY created_at',
  );
  ...
  for (const a of admins.rows) {
    ...
    if (a.is_super_admin) {
      ...
      const r = await pool.query(
        'SELECT permissions, is_super_admin FROM roles WHERE id = $1',
        [a.role],
      );
      if (!r.rows[0]) {
  ...
  for (const t of tests) {
    console.log(
      `  ${t}: ${matchPerm(marketingPerms, t) ? '✓ autorisé' : '✗ refusé'}`,
    );
  }
