...
async function main() {
  ...
  // 1. Roles
  const [roleSuperAdmin] = await db.insert(schema.roles).values({
    label: 'Super Admin',
  ...
  await db.insert(schema.roles).values([
    { label: 'Marketing', description: 'Gestion des coupons, affiliés et campagnes', permissions: ['coupons.*', 'affiliates.*', 'analytics.read'], isSuperAdmin: false },
    { label: 'Modérateur', description: 'Modération des produits et gestion des retours', permissions: ['products.moderate', 'returns.*', 'products.read'], isSuperAdmin: false },
    { label: 'Support Client', description: 'Gestion des commandes, retours et clients', permissions: ['orders.*', 'returns.*', 'customers.read'], isSuperAdmin: false },
    { label: 'Finance', description: 'Gestion des versements et commissions', permissions: ['payouts.*', 'commissions.*', 'stores.read'], isSuperAdmin: false },
  ])
  ...
  await db.insert(schema.banners).values([
    ...
    // ── Bannières écran Compte (CMS) ──
    {
      ...
      description: 'Profitez des meilleures offres de la saison sur ExpressAfri',
    ...
    },
    {
      ...
      description: 'Combinez vos achats et économisez davantage avec nos offres groupées',
  ...
  await db.insert(schema.appSettings).values([
    ...
    { key: 'app.footer',      value: '© 2026 ExpressAfri. Tous droits réservés.', type: 'text', label: 'Pied de page', group: 'general' },
    { key: 'app.version',     value: '1.0.0',        type: 'text',   label: 'Version application',   group: 'general' },
    ...
    { key: 'brand.showName',   value: 'true',    type: 'boolean', label: "Afficher le nom à côté du logo (en-tête)", group: 'theme' },
    { key: 'brand.nameColor1', value: '#FF6B35', type: 'color',   label: 'Couleur du nom — début du dégradé', group: 'theme' },
    { key: 'brand.nameColor2', value: '#E8590C', type: 'color',   label: 'Couleur du nom — fin du dégradé', group: 'theme' },
    // Badges et étiquettes (bandeaux promo gérés dans l'onglet Bannières)
    { key: 'promo.dealLabel',     value: '80% de réduction à durée limitée', type: 'text', label: 'Label Deal du Jour', group: 'promo' },
    { key: 'promo.bundleLabel',   value: '-3 $ sur 3 articles', type: 'text', label: 'Label Offres groupées', group: 'promo' },
    { key: 'promo.freeShip',      value: 'Livraison gratuite dès 3 articles', type: 'text', label: 'Texte livraison gratuite', group: 'promo' },
    { key: 'home.heroTitle',      value: 'Bienvenue sur ExpressAfri', type: 'text', label: 'Titre héro page accueil', group: 'content' },
    { key: 'home.heroSubtitle',   value: 'L\'e-commerce africain', type: 'text', label: 'Sous-titre héro page accueil', group: 'content' },
    ...
    { key: 'commerce.baseShippingFee',       value: '1000',  type: 'number', label: 'Frais de livraison de base (FCFA)', group: 'commerce' },
    ...
    // Fidélité
    { key: 'loyalty.bonusValue',      value: '20',   type: 'number', label: 'Valeur du bonus fidélité (%)', group: 'loyalty' },
  ])
  ...
  console.log('   Compte admin créé. Changez le mot de passe via l\'interface admin avant toute mise en production.')
  await pool.end()
}
