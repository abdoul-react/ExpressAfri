...
describe('Production readiness e2e (Phase F) - skeleton', () => {
  ...
  it('price falsification scenario - server recalculates totals (e2e)', async () => {
    ...
    if (
      ...
      const q = await db.execute(
        `SELECT id FROM addresses WHERE customer_id = '${userId}' LIMIT 1`,
      );
    ...
    const orderRow =
      orderRows && orderRows.rows && orderRows.rows[0]
        ? (orderRows.rows[0] as any)
        : (orderRows as any)[0];
    ...
    const itemsTotal =
      itemsRows && itemsRows.rows && itemsRows.rows[0]
        ? Number((itemsRows.rows[0] as any).items_total)
        : Number((itemsRows as any)[0].items_total);
    ...
    const expectedTotal =
      ...
      Number(orderRow.discount_amount || 0);
  ...
  it('double checkout idempotency scenario - concurrent createOrder creates single order (e2e)', async () => {
    ...
    const count =
      rows && rows.rows && rows.rows[0]
        ? Number((rows.rows[0] as any).count)
        : Number((rows as any)[0].count);
    expect(count).toBe(1);
  }, 30000);
  ...
  it('stock concurrency scenario - two concurrent orders on last stock unit result in single fulfillment (e2e)', async () => {
    ...
    const reg1 = await request(app.getHttpServer())
      .post('/mobile/auth/register')
      .send({ firstName: 'A', lastName: 'One', email: email1, password: pwd })
      .expect(201);
    const reg2 = await request(app.getHttpServer())
      .post('/mobile/auth/register')
      .send({ firstName: 'B', lastName: 'Two', email: email2, password: pwd })
      .expect(201);
    ...
    const SYSTEM_STORE_ID = '00000000-0000-0000-0000-000000000001';
    await db.execute(
      `INSERT INTO stores (id, name, email, country, status, created_at, updated_at) VALUES ('${SYSTEM_STORE_ID}', 'System store', 'system@example.com', 'Niger', 'active', now(), now()) ON CONFLICT (id) DO NOTHING`,
    );

    // create addresses for both users
    await db.execute(
      `INSERT INTO addresses (customer_id, store_id, contact_name, phone, street, city, country_code, created_at, updated_at) VALUES ('${user1}', '${SYSTEM_STORE_ID}', 'User One', '000', 'Street X', 'Niamey', 'NE', now(), now())`,
    );
    await db.execute(
      `INSERT INTO addresses (customer_id, store_id, contact_name, phone, street, city, country_code, created_at, updated_at) VALUES ('${user2}', '${SYSTEM_STORE_ID}', 'User Two', '000', 'Street Y', 'Niamey', 'NE', now(), now())`,
    );
    ...
    const variantId = randomUUID();
    await db.execute(
      `INSERT INTO products (id, store_id, name, slug, price, status, created_at, updated_at) VALUES ('${productId}','${SYSTEM_STORE_ID}','Scarce Product','scarce-product','30.00','active', now(), now())`,
    );
    await db.execute(
      `INSERT INTO product_variants (id, product_id, store_id, sku, label, price, stock, created_at, updated_at) VALUES ('${variantId}','${productId}','${SYSTEM_STORE_ID}','SKU-SCARCE','Default','30.00', 1, now(), now())`,
    );
    ...
    const payload1 = {
      ...
      shippingAddressId: (
        await db.execute(
          `SELECT id FROM addresses WHERE customer_id='${user1}' LIMIT 1`,
        )
      ).rows[0].id,
      paymentMethod: 'card',
      idempotencyKey: `s1-${Date.now()}`,
    };
    const payload2 = {
      items: [{ productId, variantId, quantity: 1 }],
      shippingAddressId: (
        await db.execute(
          `SELECT id FROM addresses WHERE customer_id='${user2}' LIMIT 1`,
        )
      ).rows[0].id,
      paymentMethod: 'card',
    ...
    const rows = await db.execute(
      `SELECT count(*) as count FROM order_items WHERE variant_id = '${variantId}'`,
    );
    const count =
      rows && rows.rows && rows.rows[0]
        ? Number((rows.rows[0] as any).count)
    ...
    const remaining =
      sv && sv.rows && sv.rows[0]
        ? Number((sv.rows[0] as any).stock)
        : Number((sv as any)[0].stock);
  ...
  it('webhook replay scenario - repeated PSP webhooks are idempotent (e2e)', async () => {
    ...
    const addrRow = await db.execute(
      `INSERT INTO addresses (customer_id, store_id, contact_name, phone, street, city, country_code, created_at, updated_at) VALUES ('${userId}', '${SYSTEM_STORE_ID}', 'Webhook', '000', 'Street W', 'Niamey', 'NE', now(), now()) RETURNING id`,
    );
    const shippingAddressId =
      addrRow && addrRow.rows && addrRow.rows[0]
        ? (addrRow.rows[0] as any).id
        : (addrRow as any)[0].id;
    ...
    const variantId = randomUUID();
    await db.execute(
      `INSERT INTO products (id, store_id, name, slug, price, status, created_at, updated_at) VALUES ('${productId}','${SYSTEM_STORE_ID}','Webhook Product','webhook-product','10.00','active', now(), now())`,
    );
    await db.execute(
      `INSERT INTO product_variants (id, product_id, store_id, sku, label, price, stock, created_at, updated_at) VALUES ('${variantId}','${productId}','${SYSTEM_STORE_ID}','SKU-WEB','Default','10.00', 10, now(), now())`,
    );
    ...
    const orderId =
      orderResp.body.id ||
      orderResp.body.order?.id ||
      orderResp.body.orderId ||
      orderResp.body.data?.id;

    // create payment row referencing this order and a unique providerPaymentId for this test
    const amount =
      orderResp.body.total ?? orderResp.body.order?.total ?? '10.00';
    const currency = 'XOF';
    ...
    expect(
      r1.body.status === 'processed' || r1.body.status === 'ignored',
    ).toBeTruthy();
    expect(
      r2.body.status === 'ignored' || r2.body.status === 'processed',
    ).toBeTruthy();
    ...
    while (Date.now() - start < maxWaitMs) {
      const pr = await db.execute(
        `SELECT status, webhook_event_id FROM payments WHERE id = '${payment.id}' LIMIT 1`,
      );
      prow = pr && pr.rows && pr.rows[0] ? pr.rows[0] : (pr as any)[0];
      if (
        prow &&
        prow.status === 'captured' &&
        prow.webhook_event_id === payload.eventId
      )
        break;
