...
ALTER TABLE "receipt_settings" ADD COLUMN IF NOT EXISTS "brand_name" text;--> statement-breakpoint
ALTER TABLE "receipt_settings" ADD COLUMN IF NOT EXISTS "logo_url" text;--> statement-breakpoint
ALTER TABLE "receipt_settings" ADD COLUMN IF NOT EXISTS "show_barcode" boolean DEFAULT false;--> statement-breakpoint
ALTER TABLE "receipt_settings" ADD COLUMN IF NOT EXISTS "accent_color" text DEFAULT '#f97316';--> statement-breakpoint
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'push_tokens_customer_id_customers_id_fk') THEN
    -- removed duplicate constraint push_tokens_customer_id_customers_id_fk (kept in 0015_push_tokens.sql)
  END IF;
END $$;
