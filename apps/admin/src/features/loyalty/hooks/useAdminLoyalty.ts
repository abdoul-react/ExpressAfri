...
export function useAdjustPoints() { const qc = useQueryClient(); return useMutation({ mutationFn: ({ customerId, balance, reason }: { customerId: string; balance: number; reason: string }) => adminLoyaltyService.adjustPoints(customerId, balance, reason), onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin', 'loyalty'] }) } }) }
